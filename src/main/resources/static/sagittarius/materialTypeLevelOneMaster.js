...
$(function () {
    ...
    $('#dg').datagrid({
        fit: true,
        toolbar: '#toolbar',
        url: '/materialTypeLevelOneMaster/load',
        columns: [[{
            field: 'id',
            title: '主键',
            hidden: true,
...
function removeUnit() {
    ...
    if (row) {
        $.messager.confirm('Confirm', '您是否确定要删除此条数据?(' + row.code + ')', function (r) {
            if (r) {
                $.post('/materialTypeLevelOneMaster/remove', {id: row.id}, function (result) {
                    if (result.success) {
...
function saveUser() {
    ...
    var realurl;
    if (editType == 'new') {
        realurl = '/materialTypeLevelOneMaster/add';
    } else if (editType == 'edit') {
        realurl = '/materialTypeLevelOneMaster/edit';
    }
    $('#fm').form('submit', {
        url: realurl,
        onSubmit: function () {
